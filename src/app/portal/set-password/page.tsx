...
import { setPortalPassword } from "@/app/actions";

const initialState: { error?: string } = {};

// Reached from an invite or password-reset email after /auth/callback has
// already exchanged the link's code for a real session — so this page runs
// as an authenticated user setting/changing their own password, not as
// part of the sign-in flow itself.
export default function SetPortalPasswordPage() {
  const [state, formAction, pending] = useActionState(
    async (_prev: typeof initialState, formData: FormData) =>
      setPortalPassword(formData),
    initialState
  );
    ...
    <div className="max-w-sm mx-auto space-y-6">
      <div>
        ...
        <p className="text-foreground/60 mt-1">
          Choose a password for signing in from now on — no email link
          needed next time.
        </p>
      </div>
